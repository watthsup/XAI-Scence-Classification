...
def get_dataset(root_dir, is_train=True, lookup_csv=cfg_file['lookup_path']):
    if is_train:
        transform = A.Compose(
            [
    ...
    train_dataset = SceneDataset(root_dir, is_train=is_train, transform=transform, lookup_csv=lookup_csv)
...
def trainKfold(hyperparams, K=5):
    ...
    for fold, (train_ids, val_ids) in enumerate(kfold.split(train_dataset)):
        ...
        with wandb.init(project="Scene-Classification", name=exp_name, config=hyperparams):
            config = wandb.config
...
if __name__ == "__main__":
    ...
    for backbone in ['inception_resnet_v2', 'resnet50', 'resnext50_32x4d', 'densenet121', 'xception', 'vgg16']:
        hyperparams = dict(
                backbone=backbone,
